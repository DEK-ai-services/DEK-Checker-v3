...
const AIChecker = ({ sheetId, onBack }) => {
    ...
    const fetchSheetData = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`/get_sheet_data?sheet_id=${sheetId}`);
            ...
            showSnackbar(
                "Nepodařilo se načíst data tabulky. Zkuste to prosím znovu.",
                "error"
            );
        } finally {
            setIsLoading(false);
    ...
    const fetchLastResponses = useCallback(
        async (page = 1) => {
            if (!productNameColumn || !analysisColumn || !selectedAssistant) {
                console.log(
                    "Missing required parameters for fetchLastResponses"
                );
                return;
            }

            try {
                const url = `/get_last_responses?sheet_id=${sheetId}&product_name_column=${encodeURIComponent(
                    productNameColumn
                )}&analysis_column=${encodeURIComponent(
                    analysisColumn
                )}&assistant_id=${selectedAssistant}&page=${page}&per_page=10`;
                ...
                showSnackbar(
                    "Nepodařilo se načíst poslední odpovědi. Zkuste to prosím znovu.",
                    "error"
                );
            }
        },
        ...
        return (
            <Box mt={2}>
                <Box
                    ...
                    mb={2}
                >
                    <Typography variant="h6">GPT kontrola</Typography>
                    {isLoading && (
                        <Box
                            display="flex"
                            alignItems="center"
                ...
                {isLoading && totalExpectedResponses > 0 && (
                    <ProgressBar
                        progress={
                            (lastResponses.length / totalExpectedResponses) *
                            100
                        }
                    />
                )}
                ...
                {visibleResponses.map((response, index) => {
                    ...
                    const rowData = sheetData.find(
                        (row) =>
                            row[productNameColumn]?.toLowerCase().trim() ===
                            response.product_name.original?.toLowerCase().trim()
                    );

                    const itemNumber = rowData
                        ? rowData["Číslo položky"]
                        : "N/A";
    ...
    const fetchBacklogResponses = useCallback(async () => {
        try {
            ...
            if (data.status === "success") {
                ...
                showSnackbar(
                    `Načteno ${data.responses.length} odpovědí z backlogu`,
                    "success"
                );
            } else {
                throw new Error(
                    data.message || "Nepodařilo se načíst backlog odpovědi"
                );
            }
        } catch (error) {
            console.error("Error fetching backlog responses:", error);
            showSnackbar(
                "Nepodařilo se načíst backlog odpovědi. Zkuste to prosím znovu.",
                "error"
            );
        }
    }, [showSnackbar]);
        ...
        try {
            const response = await fetch("/save_gpt_response", {
                ...
                body: JSON.stringify({
                    sheet_id: sheetId,
                    product_name:
                        result.product_name.original || result.product_name,
            ...
            if (data.status === "success") {
                showSnackbar(
                    "GPT odpověď byla úspěšně uložena do backlogu",
                    "success"
                );
            }
        } catch (error) {
            ...
            showSnackbar(
                "Chyba při ukládání GPT odpovědi: " + error.message,
                "error"
            );
        }
    };
    ...
    const handleAnalyze = useCallback(() => {
        ...
        const rowsToAnalyze = sheetData.filter(
            (row) => row[productNameColumn] && row[analysisColumn]
        ).length;
        setTotalExpectedResponses(rowsToAnalyze);

        const eventSource = new EventSource(
            `/analyze?sheet_id=${sheetId}&product_name_column=${encodeURIComponent(
                productNameColumn
            )}&analysis_column=${encodeURIComponent(
                analysisColumn
            )}&assistant_id=${selectedAssistant}`
            ...
            try {
                ...
                if (result.status === "error") {
                    ...
                    const formattedResult = {
                        id: result.id,
                        product_name:
                            typeof result.product_name === "object"
                                ? result.product_name.original
                                : result.product_name,
                        product_description: {
                            original:
                                result.product_description?.original || "",
                            improved:
                        ...
                        versions: [
                            {
                                ...
                                improved_text:
                                    result.product_description?.improved || "",
                                changes:
                                    result.product_description?.changes || [],
                ...
                setError(
                    "Chyba při zpracování výsledku analýzy: " + error.message
                );
                showSnackbar(
                    "Došlo k chybě při zpracování výsledků analýzy.",
                    "error"
                );
            }
        };
    ...
    const handleReject = async (result) => {
        try {
            ...
            if (!response.ok) {
                const data = await response.json();
                throw new Error(
                    data.message || "Nepodařilo se aktualizovat stav odpovědi"
                );
            }

            setLastResponses((prev) =>
                prev.filter((response) => response.id !== result.id)
            );
            ...
            showSnackbar(
                `Chyba při zamítnutí návrhu: ${error.message}`,
                "error"
            );
        }
    };
    ...
    const handleConfirm = async (result, selectedVersion) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    data.error || "Nepodařilo se aktualizovat data"
                );
            }
            ...
            showSnackbar(
                `Chyba při aktualizaci dat: ${error.message}`,
                "error"
            );
        }
    };
    ...
    const renderStatistics = () => {
        if (!sheetData.length)
            return <Typography>Žádná data k zobrazení</Typography>;

        const rowCount = sheetData.length;
        const columnCount = columns.length;
        const nonEmptyCells = sheetData.reduce(
            (acc, row) =>
                acc + Object.values(row).filter((cell) => cell !== "").length,
            0
        );
                    ...
                    <TableHeadStyled>
                        <TableRow>
                            <TableCellStyled padding="checkbox">
                                <Checkbox
                                    indeterminate={
                                        Object.values(checkedRows).some(
                                            Boolean
                                        ) &&
                                        !Object.values(checkedRows).every(
                                            Boolean
                                        )
                                    }
                                    checked={Object.values(checkedRows).every(
                                        Boolean
                                    )}
                                    onChange={() => {
        ...
        <Root>
            ...
            <Box sx={{ p: 3 }}>
                ...
                {activeTab === "statistics" && (
                    <Container maxWidth="lg">{renderStatistics()}</Container>
                )}
                {activeTab === "table" && (
                    <Container maxWidth={false}>{renderTable()}</Container>
                )}
                {activeTab === "analysis" && (
                        ...
                        >
                            ...
                            >
                                <FormControl fullWidth>
                                    <InputLabel>
                                        Sloupec s názvem produktu
                                    </InputLabel>
                                    <Select
                                        value={productNameColumn}
                                        onChange={(e) =>
                                            setProductNameColumn(e.target.value)
                                        }
                                    >
                                        {columns
                                            .sort((a, b) =>
                            ...
                            >
                                <FormControl fullWidth>
                                    ...
                                    <Select
                                        ...
                                        onChange={(e) =>
                                            setAnalysisColumn(e.target.value)
                                        }
                                    >
                                        {columns
                                            .sort((a, b) =>
